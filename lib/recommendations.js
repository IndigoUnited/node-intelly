...
function top(rankings, target, n, similarityFn, callback) {
    ...
    async.forEachOf(rankings, function __handleEntityRankings(entityRankings, key, next) {
        ...
        async.sortBy(scores, function __handleCompare(x, next) {
            return next(null, -1 * x.score); // descending order
        }, function __handleSort(err, result) {
            if (err) {
                return callback(err);
            }
